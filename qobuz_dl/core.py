...
class QobuzDL:
    ...
    def handle_oauth_login(self, code=None):
        ...
        usr_info = self.initialize_client_with_oauth(code, self.app_id, self.secrets, self.private_key)

        logger.info(f"{GREEN}OAuth login successful!")

        # Return token info so cli can save to config
        return {
            "user_id": str(usr_info["user"]["id"]),
            "user_auth_token": self.client.uat,
        }
